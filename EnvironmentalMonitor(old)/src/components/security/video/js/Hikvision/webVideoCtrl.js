                        ...
                        if (re()) {
                            ...
                            if (n.$XML(r).find("SelectWnd", !0).length > 0) {
                                ...
                                t.push("<RealPlayInfo>"),
                                    ...
                                    t.push("</RealPlayInfo>"),
                                    //改变海康的摄像头窗口号返回的参数
                                    // alert(parseInt($(e).find("SelectWnd").eq(0).text(), 10))
                                    a.cbSelWnd && a.cbSelWnd(parseInt($(e).find("SelectWnd").eq(0).text(), 10))
                    ...
                    F = function() {
                        ...
                        if (f.browser().msie) e = "<object classid='" + a.szOcxClassId + "' codebase='' standby='Waiting...' id='" + o + "' width='" + r + "' height='" + s + "' align='center' ><param name='wndtype' value='" + a.iWndowType + "'><param name='playmode' value='" + a.iPlayMode + "'><param name='colors' value='" + a.szColorProperty + "'></object>";
                            ...
                            for (var t = navigator.mimeTypes.length,
                                    n = 0; t > n; n++) navigator.mimeTypes[n].type.toLowerCase() == a.szMimeTypes && (e = "<embed windowlessVideo='1' wmode='opaque' style='position: absolute;z-index:-999;' align='center' type='" + a.szMimeTypes + "' width='" + r + "' height='" + s + "' name='" + i + "' wndtype='" + a.iWndowType + "' playmode='" + a.iPlayMode + "' colors='" + a.szColorProperty + "'>");
                        return e
                    },
                    U = function() {
                        if (!re() && null !== c) {
                        ...
                        f.extend(o, t),
                            f.extend(o, {
                                success: function(e) {
                                    ...
                                    for (var s = n.$XML(e).find("InputProxyChannel", !0), o = 0, i = s.length; i > o; o++) {
                                        let a = s[o];
                                        let id = n.$XML(a).find("id").eq(0).text()
                                        let name = f.escape(n.$XML(a).find("name").eq(0).text())
                                        r.push({
                                            id: id,
                                            name: name
                                        })
                        ...
                        f.extend(d, r),
                            f.extend(d, {
                                success: function(e) {
                                    ...
                                    // t.push("<CMSearchResult>"),
                                    // t.push("<responseStatus>" + n.$XML(e).find("responseStatus").eq(0).text() + "</responseStatus>"),
                                    t.responseStatus = n.$XML(e).find("responseStatus").eq(0).text()
                                        // t.push("<responseStatusStrg>" + n.$XML(e).find("responseStatusStrg").eq(0).text() + "</responseStatusStrg>"),
                                    t.responseStatusStrg = n.$XML(e).find("responseStatusStrg").eq(0).text()
                                        // t.push("<numOfMatches>" + n.$XML(e).find("numOfMatches").eq(0).text() + "</numOfMatches>"),
                                    t.numOfMatches = n.$XML(e).find("numOfMatches").eq(0).text()
                                        // t.push("<matchList>");
                                    t.matchList = {};
                                    t.matchList.searchMatchItem = []
                                    for (var s = n.$XML(e).find("searchMatchItem", !0), o = 0, i = s.length; i > o; o++) {
                                        var a = s[o];
                                        // t.push("<searchMatchItem>"),
                                        t.matchList.searchMatchItem[o] = {}
                                            // t.push("<trackID>" + n.$XML(a).find("trackID").eq(0).text() + "</trackID>"),
                                        t.matchList.searchMatchItem[o].trackID = n.$XML(a).find("trackID").eq(0).text()
                                            // t.push("<startTime>" + n.$XML(a).find("startTime").eq(0).text() + "</startTime>"),
                                        t.matchList.searchMatchItem[o].startTime = n.$XML(a).find("startTime").eq(0).text()
                                            // t.push("<endTime>" + n.$XML(a).find("endTime").eq(0).text() + "</endTime>"),
                                        t.matchList.searchMatchItem[o].endTime = n.$XML(a).find("endTime").eq(0).text()
                                            // t.push("<playbackURI>" + f.escape(n.$XML(a).find("playbackURI").eq(0).text()) + "</playbackURI>"),
                                        t.matchList.searchMatchItem[o].playbackURI = f.escape(n.$XML(a).find("playbackURI").eq(0).text())
                                            // t.push("<metadataDescriptor>" + n.$XML(a).find("metadataDescriptor").eq(0).text().split("/")[1] + "</metadataDescriptor>"),
                                        t.matchList.searchMatchItem[o].metadataDescriptor = n.$XML(a).find("metadataDescriptor").eq(0).text().split("/")[1]
                                            // t.push("</searchMatchItem>")
                                    }
                                    // t.push("</matchList>"),
                                    // t.push("</CMSearchResult>"),

                                    // console.log("le.prototype.recordSearch.t:"+t)
                                    // e = f.loadXML(t.join("")),
                                    // console.log("le.prototype.recordSearch.e:"+e)
